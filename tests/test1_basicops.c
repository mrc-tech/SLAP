...
int main()
{
	...
	*/
	...
	printf("mat_mul       : ");
	tmp = mat_init(3,3, (double[]){30,36,42,66,81,96,102,126,150});
	if(mat_equal(mat_mul(A,A),tmp,0))					printf("passed\n"); else printf("NOT PASSED!!!\n");
	u = mat_init(1,3, (double[]){1,2,3});
	v = mat_init(1,3, (double[]){2,2,2});
	tmp = mat_init(1,1, (double[]){12}); // scalar product (vectors)
	if(mat_equal(mat_mul(u,mat_transpose(v)),tmp,0)) printf("              : passed\n"); else printf("                 NOT PASSED!!!\n");
	tmp = mat_init(3,3, (double[]){2,2,2,4,4,4,6,6,6}); // tensor product (vectors)
